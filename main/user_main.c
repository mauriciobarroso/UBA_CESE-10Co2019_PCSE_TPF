...
static void task_show( void *arg )
{
	...
	for( ;; )
	{
		...
		if( xStatus == pdPASS )
		{
			...
			{
				case dRtcTimeDate:
					/* envía al LCD los datos del RTC */
					lcd_gotoxy( 0, 0 );
					snprintf( buffer, 14, "Date:%02x/%02x/%02x\r", data[ DS1307_DATE_ADDR ], data[ DS1307_MONTH_ADDR ], data[ DS1307_YEAR_ADDR ] );
			 		lcd_puts( buffer );
			 		lcd_gotoxy( 0, 1 );
			 		snprintf( buffer, 14, "Time:%02x:%02x:%02x\r\n", data[ DS1307_HOURS_ADDR ], data[ DS1307_MINUTES_ADDR ], data[ DS1307_SECONDS_ADDR ] );
			 		lcd_puts( buffer );
			 		break;
			 	case dEepromData:
			 		/* envía al LCD el conteo de pulsos almacenado en la EEPROM */
			 		lcd_gotoxy( 0, 2 );
			 		snprintf( buffer, 11, "Pulsos:%03d\r\n", (int) data );
			 		lcd_puts( buffer );
			 		at24c32_read_random( EEPROM_PULSE_COUNT, &eeprom );
			 		lcd_gotoxy( 0, 3 );
			 		snprintf( buffer, 18, "PULSE_COUNT=0x%02X", eeprom );
			 		lcd_puts( buffer );
			 		break;
			 }
		}
